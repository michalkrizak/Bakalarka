...
Jako technologický základ byl zvolen framework **Selenium WebDriver**, který je v .NET prostředí standardem pro automatizaci webových prohlížečů. Pro samotné spouštění testů a validaci podmínek (assertions) je využit framework **NUnit** (atributy \texttt{[Test]}, \texttt{[SetUp]}).

\subsection{Architektura Screenplay Pattern}
Na rozdíl od tradičního přístupu Page Object Model (POM) využívá tento projekt modernější návrhový vzor **Screenplay Pattern**, implementovaný pomocí knihovny \textbf{Boa.Constrictor}. Tento přístup se zaměřuje na to, co uživatel (Actor) dělá, spíše než na strukturu stránek.
...
Nedílnou součástí mé práce byla integrace těchto testů do procesu kontinuální integrace a nasazování (CI/CD) na platformě \textbf{Azure DevOps}.
