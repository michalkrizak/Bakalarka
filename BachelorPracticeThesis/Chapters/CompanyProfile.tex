...
Tato kapitola představuje společnost AVE Soft s.r.o., ve které probíhala odborná praxe, a popisuje pracovní zařazení v rámci vývojového týmu.
...
Sídlo společnosti se nachází v technologickém areálu v Ostravě-Pustkovci, což umožňuje úzkou spolupráci s akademickou sférou, zejména s VŠB – Technickou univerzitou Ostrava. Firma klade důraz na využití moderních technologií, jako jsou cloudové služby Microsoft Azure, platforma .NET \cite{dotnet_docs} a frameworky pro vývoj webových aplikací (Angular).

\section{Pracovní zařazení a role v týmu}

V rámci odborné praxe jsem byl zařazen do vývojového týmu jako programátor. Moje práce probíhala pod vedením zkušeného konzultanta (mentora) z firmy. Hlavním cílem mého působení byla práce na projektu \textbf{Importér}, který je součástí širšího ekosystému Evolio.

\subsection*{Náplň práce}
Moje činnost byla rozdělena do několika klíčových oblastí:

\begin{itemize}
    \item \textbf{Vývoj frontendu (FE):} Práce na uživatelském rozhraní v technologii \textbf{Angular} \cite{angular_docs} a jazyce \textbf{TypeScript} \cite{typescript_docs}. Hlavním úkolem bylo vytvoření moderní Single Page Application (SPA), která nahrazuje starší desktopové řešení.
    \item \textbf{Vývoj backendu (BE):} Implementace doplňku (Add-in) pro Importér v jazyce \textbf{C\#} \cite{csharp_docs}, který zajišťuje logiku zpracování dat a komunikaci s databází.
    ...
    \item \textbf{Řešení FE issues:} Údržba a rozvoj existujících částí aplikace mimo hlavní projekt. Významným úkolem v této oblasti byla implementace uživatelského rozhraní pro AI asistenta (chat), která zahrnovala výběr jazykových modelů a práci s historií konverzace.
\end{itemize}
